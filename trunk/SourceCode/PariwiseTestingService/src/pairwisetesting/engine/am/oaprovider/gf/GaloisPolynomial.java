...
/**
 * A class for working with Polynomials. <BR>
...
public class GaloisPolynomial {
	 ...
	 * @param newalpha
	 *            The variable. For ex: 'x'.
	 * @param newfield
	 *            The coefficient galois field.
	 */
	public GaloisPolynomial(int[] newcoef, char newalfa, GaloisField newfield) {
		...
		while (len > 0 && field.normalise(newcoef[len - 1]) == GaloisField.ZERO)
			len--;
	 ...
	 * 
	 * @param poly
	 *            A string representation for the polynomial.
	 * @param newalpha
	 *            The variable. For ex: 'x'.
	 * @param newfield
	 *            The coefficient galois field.
	 ...
	 *            The coefficient galois field.
	 */
	public static GaloisPolynomial createConstant(int elem, char newalfa,
	...
	/**
	 * Evaluates the polynomial for a given galois element using the Horner
	 * scheme.
	 */
	...
	/**
	 * Returns a string representation for this polynomial. <BR>
	 * For ex: 1 + 3x + 4x^2.
	 */
	...
	/**
	 * Returns a string representation for this polynomial. <BR>
	 * For ex: 11010001011 GF(2) (only the coefficients are given). <BR>
	...
	/**
	 * Check if two polynomials are equal. That is, if the coefficients are
	 * equal.
	 */
	public boolean equals(GaloisPolynomial b) {
		...
		for (int ctr = 0; ctr < coefs.length; ctr++)
			if (coefs[ctr] != b.getCoefficient(ctr))
				return false;
	...
	/**
	 * Returns the coefficient for position [pos]. <BR>
	...
	/**
	 * Returns a copy of this polynomial. <BR>
	 * You should never use this method because a polynomial can't change. <BR>
	...
	public GaloisPolynomial product(GaloisPolynomial b) {
		...
		for (int ctr = 0; ctr <= degres; ctr++) {
			...
			for (int deg = ctr; deg >= 0; deg--) {
				coef = field.sum(coef, field.product(getCoefficient(deg), b
						.getCoefficient(ctr - deg)));
			}
			prodcoef[ctr] = coef;
		}
	...
	/**
	 * Returns the product of 'this' and a 'one term polynomial'. <BR>
	 * For ex: [coef]x^[pwr]
	 * 
	 * @param coef
	 *            The coefficient for the one-term.
	 * @param pwr
	 *            The power for the one-term.
	 */
	...
	private static int[] createCoefficients(int deg) {
		...
		for (int ctr = 0; ctr < deg; ctr++)
			rescoef[ctr] = GaloisField.ZERO;
		return rescoef;
	}

	private static void incCoefficients(int[] rescoef, int deg,
			GaloisField field) {
		int idx, pos, prevpos;
		boolean carry;
	...
	/**
	 * Returns a vector containing all the possible irreducible monomials. <BR>
	...
	public static Vector irrMonomials(int deg, char alfa, GaloisField field) {
		...
		for (crdeg = 2; crdeg <= deg; crdeg++) {
			...
			for (ctr = 0; ctr < num; ctr++) {
				...
				irr = true;
				GaloisPolynomial thispoly = new GaloisPolynomial(rescoef, alfa,
						field);
				...
				for (ldeg = 2; ldeg < crdeg && irr; ldeg++) {
					for (idx = 0; idx < calc[ldeg - 2].size() && irr; idx++) {
						try {
							GaloisPolynomial ldegpoly = (GaloisPolynomial) calc[ldeg - 2]
									.elementAt(idx);
							GaloisPolynomial[] divres = thispoly
									.divide(ldegpoly);
							if (divres[1].equals(zeropol))
								irr = false;
				...
				if (irr)
					calc[crdeg - 2].addElement(thispoly);
			}
		}
		return calc[deg - 2];
	}
	 ...
	 *            The galois field.
	 */
	public static GaloisPolynomial[] allPolynomials(int deg, char alfa,
			GaloisField field) {
		int card = field.getCardinality();
	...
	private static int[] parsexpwrs(int mnpwr, int mxpwr, String poly,
			char newalfa, GaloisField newfield) throws GaloisException {
		int pos = -1;
		int len = poly.length();
		int ch, stpos, pwr, xpos, level, mpos;
		int[] coefs = new int[mxpwr + 1];
		boolean[] pwrused = new boolean[mxpwr];
		for (int ctr = 0; ctr < mxpwr; ctr++)
			pwrused[ctr] = false;
		...
		while (pos < len && (pos = poly.indexOf(newalfa, pos + 1)) != -1) {
			...
			if (pos < len) {
				...
				if (pos < len) {
					...
					if (pos < len && poly.charAt(pos) != '+')
						throw new GaloisException("Parse: x^" + pwr
								+ " must be followed by + or EOS",
			...
			if (level != 0)
				throw new GaloisException("Parse: () don't match",
						GaloisException.PARSE);
			if (mpos != 0)
				mpos++;
			// Watch out for power !
			if (pwrused[pwr - 1])
				throw new GaloisException("Parse: Two times x^" + pwr
						+ " encountered", GaloisException.PARSE);
			pwrused[pwr - 1] = true;
			// Nu is alles vanaf mpos tot xpos-1 de bijhorende coefficient.
			coefs[pwr] = newfield.parse(poly.substring(mpos, xpos),
					GaloisField.ONE);
			if (pwr == mnpwr && mpos != 0)
				coefs[0] = newfield.parse(poly.substring(0, mpos - 1),
						GaloisField.ZERO);
		}
		return coefs;
	}
	...
	private static int[] parsespaces(String poly, GaloisField newfield)
		...
		nowpwr = 0;
		while (pos < len && (pos = poly.indexOf(' ', pos + 1)) != -1) {
			coefs[nowpwr] = newfield.parse(poly.substring(prevpos, pos),
					GaloisField.ONE);
			nowpwr++;
			prevpos = pos + 1;
		}
		if (prevpos < len) {
			coefs[nowpwr] = newfield.parse(poly.substring(prevpos, len),
					GaloisField.ONE);
		}
		return coefs;
	}
	...
	private static int[] parsecoefs(String poly, char newalfa,
		...
		while (pos < len && (pos = poly.indexOf(newalfa, pos + 1)) != -1) {
				...
				if (poly.charAt(pos) == '^') {
					...
					if (ch < '0' || ch > '9')
						throw new GaloisException(
								"Parse: ^ must be followed by a number",
								GaloisException.PARSE);
					stpos = pos;
					while (pos < len - 1 && ch >= '0' && ch <= '9')
						ch = poly.charAt(++pos);
			...
			if (pwr > mxpwr)
				mxpwr = pwr;
			if (pwr < mnpwr || mnpwr == -1)
				mnpwr = pwr;
		}
		if (mxpwr > 0)
			return parsexpwrs(mnpwr, mxpwr, poly, newalfa, newfield);
		String data = GaloisField.cleanString(poly);
		if (data.indexOf(' ') != -1)
			return parsespaces(data, newfield);
		int rootcard = newfield.getRoot().getCardinality();
		if (data.length() > 1 && rootcard < 10)
			return parsedigits(data, newfield);
		int[] coefs = new int[1];
