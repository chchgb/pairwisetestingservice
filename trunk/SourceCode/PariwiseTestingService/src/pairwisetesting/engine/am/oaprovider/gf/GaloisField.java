...
public class GaloisField {
	...
	/**
	 * A constant for the display mode ALFAPOWER <BR>
	 * For ex: valueString(a) -> a^k
	 */
	public static final int ALFAPOWER = 0;
	/**
	 * A constant for the display mode POLYNOMIAL <BR>
	 * For ex: valueString(a) -> 1+a
	 */
	public static final int POLYNOMIAL = 1;
	/**
	 * A constant for the display mode POLYCOEFS <BR>
	 ...
	 * 
	 * @param p
	 *            The cardinality for the new field.
	 ...
	 *            The cardinality for the new field.
	 * @param newalpha
	 *            The index of the primitive element.
	 */
	public GaloisField(int newcard, int newalfa) {
		card = newcard;
		dmode = POLYNOMIAL;
		alfa = newalfa;
	}
	...
	/**
	 * Normalises a galois element. <BR>
	...
	/**
	 * Returns a String representation of this field. <BR>
	 ...
	 */
	public int divide(int a, int b) throws GaloisException {
		if ((b % card) == ZERO)
			throw new GaloisException("Divide by zero", GaloisException.DIVIDE);
		int c = glPower(b, card - 2);
		return (a * c) % card;
	}

	/**
	 * Returns the cyclotomic of [elem] modulo [modulo]. <BR>
	 * This is an array of primitive element powers.
	 * 
	 * @param modulo
	 *            The modulo parameter for the cyclotomic.
	 * @param elem
	 *            The starting power.
	 */
	...
	/**
	 * Returns all the cyclotomics modulo [modulo]. <BR>
	 * This is a Vector of cyclotomics. <BR>
	 * See: getCyclotomic
	 * 
	 * @param modulo
	 *            The modulo parameter for the cyclotomic.
	 */
	public Vector<int[]> getAllCyclotomics(int modulo) {
	...
	/**
	 * Method for construction of primitive roots for 'one'. <BR>
	 * Returns an array of two integers. <BR>
	 * The first is the k-parameter for the extended field containing the root.
	 * <BR>
	 * The second is the power p of the primitive element in that field. <BR>
	 * beta^n = 1, beta = alpha^p in GF(base^k).
	 * 
	 * @param n
	 *            Take the n-th root of 'one'.
	 */
	 ...
	 * 
	 * @param alphapwr
	 *            To identify the polynomial: p(alpha^alphapwr) = 0.
	 * @param talpha
	 *            The polynomial variable. For ex: 'x'.
	 * @exception GaloisException
	 *                If an error occurs.
	 */
	public GaloisPolynomial getMinimalPolynomial(int alphapwr, char talfa)
		...
		GaloisPolynomial betw = GaloisPolynomial.createConstant(
				GaloisField.ONE, talfa, this);
		int[] coefs = new int[2];
		coefs[1] = GaloisField.ONE;
		for (int ctr = 0; ctr < cyclo.length; ctr++) {
			coefs[0] = minus(GaloisField.ZERO, getAlfaPower(cyclo[ctr]));
			GaloisPolynomial factor = new GaloisPolynomial(coefs, talfa, this);
	...
	/**
	 * Converts a galois element from 'this' field to a given
	 * ExtendedGaloisField.
	 * 
	 * @exception GaloisException
	...
	/**
	 * Cleans a string. Removes all surrounding spaces and parantheses. <BR>
	 * For ex: " ( (lk()dj )) " -> "lk()dj"
	 */
	public static String cleanString(String strg) {
		...
		boolean again;
		do {
			while (psa < len && strg.charAt(psa) == ' ')
				psa++;
			while (psb > 0
					&& (strg.charAt(psb) == ' ' || strg.charAt(psb) == '*'))
				psb--;
			if (psb < psa)
				return "";
			again = false;
			if (psa < len && psb > 0 && strg.charAt(psa) == '('
					&& strg.charAt(psb) == ')') {
				psa++;
				psb--;
				again = true;
	...
	public int parse(String strg, int onempty) throws GaloisException {
		...
		int c = atoi(mystrg);
		if (c >= card)
			throw new GaloisException("parse: Digit not in root field",
	...
	private static int findAlfa(int num) throws GaloisException {
		...
		throw new GaloisException("Can't find primitive element",
				GaloisException.PRIMITIVE);
	}
	...
	public static int atoi(String intvl) throws GaloisException {
		...
		for (ctr = 0; ctr < len; ctr++) {
			...
			ch = intvl.charAt(ctr);
			if (ch < '0' || ch > '9')
				throw new GaloisException("Atoi: Character not a digit [0..9]",
						GaloisException.PARSE);
			betw += (int) (ch - '0');
		}
		return betw;
	...
	/**
	 * Calculates the power of a given integer. <BR>
