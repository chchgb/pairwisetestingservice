...
public class ExtendedGaloisField extends GaloisField {
	...
	public ExtendedGaloisField(GaloisField newbase, char newalfa, int newpwr)
		...
		for (ctr = 0; ctr < irrpolys.size() && alfafound == false; ctr++) {
			GaloisPolynomial irrpoly = (GaloisPolynomial) irrpolys
					.elementAt(ctr);
		...
		if (alfafound)
			...
			throw new GaloisException("Can't find primitive polynomial",
					GaloisException.PRIMITIVE);
	}

	public ExtendedGaloisField(GaloisPolynomial irrpoly) throws GaloisException {
		super(GaloisField.power(irrpoly.getField().getCardinality(), irrpoly
				.degree()), 0);
	...
	public int plusmin(int a, int b, boolean add) {
		...
		if (c == GaloisField.ZERO) {
			if (add)
				...
				ap = GaloisPolynomial.createConstant(GaloisField.ZERO, base
						.getAlfa(), base);
		} else
			ap = alfaPowers[c - 1];
		bp = alfaPowers[d - 1];
	...
	public int product(int a, int b) {
		...
		c = normalise(a);
		d = normalise(b);
		if (c == GaloisField.ZERO || d == GaloisField.ZERO)
			return GaloisField.ZERO;
	...
	public int toBase(int element) throws GaloisException {
		...
		if (c == GaloisField.ZERO || c == GaloisField.ONE)
			return c;
		GaloisPolynomial myelem = alfaPowers[c - 1];
		if (myelem.degree() > 0)
			throw new GaloisException("toBase: Element not in base field",
					GaloisException.FIELDCONV);
		return myelem.getCoefficient(0);
	}

	public int toSuper(int a) throws GaloisException {
		if (a == GaloisField.ZERO || a == GaloisField.ONE)
			return a;
		int res = -1;
		int mycard = getCardinality();
		GaloisPolynomial myelem = GaloisPolynomial
				.createConstant(a, alfa, base);
		for (int ctr = 0; ctr < mycard && res == -1; ctr++) {
			if (myelem.equals(alfaPowers[ctr]))
				res = ctr + 1;
		}
		if (res == -1)
			throw new GaloisException("toSuper: Element not in extended field",
					GaloisException.FIELDCONV);
		return res;
	}
	...
	public int parse(String strg, int onempty) throws GaloisException {
		...
		if (mystrg.length() == 0)
			return onempty;
		GaloisPolynomial mypoly = new GaloisPolynomial(mystrg, alfa, base);
		...
		int mycard = getCardinality();
		for (int ctr = 0; ctr < mycard; ctr++) {
			if (mypoly.equals(alfaPowers[ctr]))
				return ctr + 1;
		}
		return GaloisField.ZERO;
	}
	...
	public GaloisPolynomial[] getAlfaPowers() {
		return alfaPowers;
	}
}
...
class ExtendedFieldHelper {
	...
	public ExtendedFieldHelper(int newpwr, char newalfa, GaloisField newbase)
		...
		base = newbase;
		if (pwr < 2)
			throw new GaloisException("Modulo Polynomial Degree < 2",
	...
	private GaloisPolynomial alfaProduct(GaloisPolynomial a, GaloisPolynomial b)
			throws GaloisException {
		if (moduloPoly == null)
			throw new GaloisException("Modulo Polynomial Null",
					GaloisException.MODULOPOLY);
